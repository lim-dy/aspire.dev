...
In your AppHost project, register the Azure Blob Storage integration by chaining a call to `AddBlobs` on the `IResourceBuilder<IAzureStorageResource>` instance returned by `AddAzureBlobServiceClient`. The following example demonstrates how to add an Azure Blob Storage resource named `storage` and a blob container named `blobs`:

```csharp title="C# — AppHost.cs"
var builder = DistributedApplication.CreateBuilder(args);

var blobs = builder.AddAzureBlobServiceClient("storage")
    .AddBlobs("blobs");
...
var storageaccount = builder.AddAzureBlobServiceClient("storage")
...
var storage = builder.AddAzureBlobServiceClient("storage")
